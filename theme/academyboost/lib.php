...
function theme_academyboost_get_pre_scss($theme) {
...
// Note the function name starts with the component name because this is a global function and we don't want namespace clashes.
function theme_academyboost_get_extra_scss($theme) {
    // Load the settings from the parent.
    $theme = theme_config::load('boost');
...
function theme_academyboost_get_main_scss_content($theme) {
    global $CFG;
    ...
    } else if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_academyboost', 'preset', 0, '/', $filename))) {
        // This preset file was fetched from the file area for theme_acadenyboost and not theme_boost (see the line above).
